...
if($status==false){
  ...
  exit("ErrorQuery:".$error[2]);
    ...
    $view .= '<th class="mdl-data-table__cell--non-numeric">書籍名</th>';
  ...
  while( $result = $stmt->fetch(PDO::FETCH_ASSOC)){
    ...
    $view .= '<td class="mdl-data-table__cell--non-numeric">'.'<img src="'.$result["img"].'">'.'</td>';
    $view .= '<td class="mdl-data-table__cell--non-numeric">'.'<a href="'.$result["url"].' " target="_blank">'.$result["bookname"].'</a><br>'.$result["author"].'</td>';
